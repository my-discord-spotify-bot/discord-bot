...
function resolveSpotifyCredentials() {
  const username = process.env.SPOTIFY_BOT_EMAIL;
  const password = process.env.SPOTIFY_BOT_PASSWORD;

  if (!username || !password) {
    throw new Error("Les identifiants Spotify du bot (SPOTIFY_BOT_EMAIL / SPOTIFY_BOT_PASSWORD) sont manquants.");
  }
...
function startLibrespotProcess(userId) {
  return new Promise((resolve, reject) => {
    try {
      const { username, password } = resolveSpotifyCredentials();
      ...
      const safeArgs = args.map((arg) => (arg === password ? "***" : arg));
      console.log("[voicePlayer] Démarrage de librespot avec les arguments :", safeArgs);
